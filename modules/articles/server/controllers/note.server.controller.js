...
var path = require('path'),
  ...
  fs = require("fs"),
  j = require(path.resolve('./modules/articles/server/controllers/json2.json')),
    ...
    if (err) {
      ...
      Note.findOne(note).populate('files').exec(function(err, item) {
        ...
        if (note.followUp !== null) {
          ...
          console.log("note.assign: " + note.assign);
...
exports.read = function(req, res) {
  var query = Note.find();
  query.where("_case").equals(req.user._id).sort('-date').populate('user', 'displayName').exec(function(err, articles) {
    if (err) {
...
exports.followupPaging = function(req, res) {
  ...
  query.populate('_case');
  query.populate('id');
  console.log(req.params.start * limit);
