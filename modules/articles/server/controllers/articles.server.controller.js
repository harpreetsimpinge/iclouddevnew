...
exports.create = function (req, res) {
  ...
  article.status = "open";
  article.permissions = req.body.permissions;
  //console.log(article);
...
exports.update = function (req, res) {
  ...
  Article.update({_id : req.body._id},req.body, {upsert:false}, function(err, doc){
    console.log(err, doc);
    if (err){
...
exports.list = function (req, res) {
  ...
  if(req.body.closed === "open"){
    ...
    orArr.push({ 
      $or:
        [{'DateClosedFocusInformation.value':null},{'DateClosedFocusInformation.value':{ $gte: d1, $lte: d2 } }]
    });
  ...
  if(req.user.roles.indexOf("admin") === -1 && req.user.roles.indexOf("admin2") === -1){
    orArr.push({$or:  [{ permissions: req.user._id }, { user: req.user._id }]
    });
...
exports.articleByID = function (req, res, next, id) {
 ...
 query.select('-legacy');
  ...
  query.lean().exec(function (err, article) {
    if (err) {
      return next(err);
    } else if (!article) {
...
exports.createLegacy = function (req, res) {
  ...
  query.exec(function(err, doc){
    ...
    Note.update({"article": doc._id}, { $set: { status: 'legacy' }}, function(err, update){
...
exports.checkClaim = function (req, res) {
  ...
  //findExtra();
  var Article = mongoose.model('Article');
  Article.where("ClaimContact.value").equals(req.params.val).find(function(err, re){
...
function doSql(i, callback){
  ...
  Fields.find().exec(function(error,fields){
      ...
      query.findOne().exec(function(error,user){ 
        ...
        article.status = "open";
        article.oldCreator = js[i]['C_CREATECID'];
